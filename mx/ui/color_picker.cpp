...
void FillChecker(mx::Canvas& canvas, const RectF& r, float cell = 6.f) {
...
class ColorPopup : public Node {
 ...
 private:
  ...
  void PaintSimple(mx::Canvas& canvas, const ThemeTokens& th) {
    PaintPreview(canvas, th);
    PaintPresets(canvas, th);
  }

  void PaintFull(mx::Canvas& canvas, const ThemeTokens& th) {
    const RectF sv = SvRect();
  ...
  void PaintPreview(mx::Canvas& canvas, const ThemeTokens& th) {
    const RectF p = PreviewRect();
  ...
  void PaintPresets(mx::Canvas& canvas, const ThemeTokens& th) {
    const int n = PresetShown();
    for (int i = 0; i < n; ++i) {
...
void ColorPicker::Paint(mx::Canvas& canvas) {
  if (!visible()) {
    return;
  }
  const ThemeTokens& th = Theme::Active();
