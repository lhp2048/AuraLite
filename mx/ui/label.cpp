...
mx::TextHAlign Label::ToCanvasAlign(TextAlign a) {
  switch (a) {
...
SizeF Label::Measure(float max_w, float max_h) {
  ...
  if (wrap_) {
    ...
    for (const auto& line : lines) {
      ...
      const float w = mx::MeasureUiTextWidth(line, fs, font) + 4.f;
      if (w > hug_w) {
        hug_w = w;
      }
    }
  ...
  const float hug_w =
      text_.empty()
          ? 0.f
          : (mx::MeasureUiTextWidth(text_, fs, font) + 4.f);
  return ResolveSize(max_w, max_h, hug_w, line_h);
}

void Label::Paint(mx::Canvas& canvas) {
  ...
  const float fs = ResolveFontSize(font_size_);
  const wchar_t* font = th.font_ui.c_str();
  const mx::TextHAlign ha = ToCanvasAlign(align_);
