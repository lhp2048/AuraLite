...
RectF Submenu::AnchorScreenRect() const {
  ...
  POINT tl{static_cast<LONG>(mx::PxFromDip(bounds_.x, dpi)),
           static_cast<LONG>(mx::PxFromDip(bounds_.y, dpi))};
  ClientToScreen(w->hwnd(), &tl);
  return RectF{static_cast<float>(tl.x), static_cast<float>(tl.y),
               mx::PxFromDip(bounds_.w, dpi),
               mx::PxFromDip(bounds_.h, dpi)};
}
...
void Submenu::Paint(mx::Canvas& canvas) {
  if (!visible()) {
    return;
  }
  const ThemeTokens& th = Theme::Active();
