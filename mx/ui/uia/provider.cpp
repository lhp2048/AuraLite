 ...
 public:
  ...
  HRESULT STDMETHODCALLTYPE get_BoundingRectangle(UiaRect* pRetVal) override {
    ...
    *pRetVal = UiaRect{0, 0, 0, 0};
    ...
    POINT tl{static_cast<LONG>(std::floor(mx::PxFromDip(b.x, dpi))),
             static_cast<LONG>(std::floor(mx::PxFromDip(b.y, dpi)))};
    POINT br{static_cast<LONG>(std::ceil(mx::PxFromDip(b.x + b.w, dpi))),
             static_cast<LONG>(std::ceil(mx::PxFromDip(b.y + b.h, dpi)))};
  ...
  HRESULT STDMETHODCALLTYPE ElementProviderFromPoint(
    ...
    *pRetVal = nullptr;
    ...
    const float dx = mx::DipFromPx(static_cast<float>(pt.x), w->dpi_);
    const float dy = mx::DipFromPx(static_cast<float>(pt.y), w->dpi_);
    Node* hit = nullptr;
    if (w->popup_) {
